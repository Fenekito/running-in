 ...
 * @returns {boolean}
 */
runningIn.browser = typeof window !== 'undefined' && typeof window.document !== 'undefined';

/**
 * Checks if the code is running in a Node.js environment.
 * @returns {boolean}
 */
runningIn.node = typeof process !== 'undefined' && process.versions != null && process.versions.node != null;

/**
 * Checks if the code is running in a web worker.
 * @returns {boolean}
 */
runningIn.webWorker = typeof self === 'object' && self.constructor && self.constructor.name === 'DedicatedWorkerGlobalScope';

/**
 * Checks if the code is running in a Deno environment.
 * @returns {boolean}
 */
runningIn.deno = typeof Deno !== 'undefined' && typeof Deno.version !== 'undefined';
 ...
 * @returns {boolean}
 */
runningIn.bun = typeof Bun !== 'undefined' && typeof Bun.version !== 'undefined';
...
runningIn.serviceWorker = (() => {
    try {
        return typeof self !== 'undefined' && typeof self.importScripts === 'function' && typeof ServiceWorkerGlobalScope !== 'undefined';
    } catch (e) {
        return false;
    }
})();
...
runningIn.containerized = (() => {
    ...
    try {
        const fs = require('fs');
        return fs.existsSync('/.dockerenv') || (fs.existsSync('/proc/self/cgroup') && fs.readFileSync('/proc/self/cgroup', 'utf8').includes('docker'));
    } catch {
        return false;
    }
})();
 ...
 * @returns {boolean}
 */
runningIn.debug = runningIn.node && !!(process.env.DEBUG || process.env.DEBUG_MODE);
 ...
 * @returns {boolean}
 */ 
runningIn.development = runningIn.node && (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === undefined);
...
const envCheck = (key, value = 'true') => runningIn.node && process.env[key] === value;
...
const uaCheck = (pattern) => runningIn.browser && pattern.test(navigator.userAgent);
...
runningIn.getInfo = function() {
    const info = { environment: {}, os: {}, browser: {}, device: {}, capabilities: {}, runtime: {} };
    ...
    if (runningIn.node) {
        ...
        info.capabilities.touch = info.capabilities.webgl = info.capabilities.serviceWorker = false;
        ...
        if (platform.includes('Win')) {
            ...
            info.os.name = 'macOS';
            info.os.version = matchVersion(ua, /Mac OS X (\d+[_\d]+)/).replace(/_/g, '.');
        ...
        if (runningIn.chrome) {
            ...
            info.browser.name = info.browser.version = info.browser.engine = 'unknown';
        }

        // Device info
        info.device.type = runningIn.mobile ? 'mobile' : runningIn.tablet ? 'tablet' : 'desktop';
        ...
        info.capabilities.touch = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
        ...
        info.capabilities.sessionStorage = checkCapability(window, 'sessionStorage');
...
runningIn.getRequestInfo = function(req) {
    ...
    const info = { url: {}, headers: {}, method: '', protocol: '', client: {}, query: {}, body: {} };
    ...
    try {
        ...
        if (req.url) {
            const urlObj = new (runningIn.node ? require('url').URL : URL)(req.url, `http://${req.headers?.host || 'localhost'}`);
        ...
        if (req.headers) {
            ...
            info.headers.userAgent = req.headers['user-agent'] || req.headers['User-Agent'] || '';
            info.headers.host = req.headers['host'] || req.headers['Host'] || '';
            info.headers.contentType = req.headers['content-type'] || req.headers['Content-Type'] || '';
            info.headers.origin = req.headers['origin'] || req.headers['Origin'] || '';
            info.headers.referer = req.headers['referer'] || req.headers['Referer'] || '';
        }

        // Extract client information
        if (runningIn.node) {
            // Node.js request object
            info.client.ip = req.ip || req.headers['x-forwarded-for'] || req.connection?.remoteAddress || '';
        ...
        info.isXmlHttpRequest = req.headers?.['x-requested-with'] === 'XMLHttpRequest' || false;
